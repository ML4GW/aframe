...
class FrequencyDomainMultimodalAframeDataset(SupervisedAframeDataset):
    def build_val_batches(self, background, signals):
        ...
        X_bg_low = self.whitener(
            X_bg, psds, lowpass=self.hparams.lowpass, highpass=None
        )
        X_bg_high = self.whitener(
            X_bg, psds, lowpass=None, highpass=self.hparams.highpass
        )

        X_fg_low, X_fg_high = [], []
        for inj in X_inj:
            inj_low = self.whitener(
                inj, psds, lowpass=self.hparams.lowpass, highpass=None
            )
            inj_high = self.whitener(
                inj, psds, lowpass=None, highpass=self.hparams.highpass
            )
        ...
        if X_fg_fft.real.ndim == 4:
            inv_asds_inj = inv_asds.unsqueeze(0).expand(
                X_fg_fft.shape[0], -1, -1, -1
            )
        else:
        ...
        X_fg_fft = torch.cat(
            [X_fg_fft.real, X_fg_fft.imag, inv_asds_inj], dim=2
        )
    ...
    def augment(self, X, waveforms):
        ...
        X_low = self.whitener(
            X, psds, lowpass=self.hparams.lowpass, highpass=None
        )
        X_high = self.whitener(
            X, psds, lowpass=None, highpass=self.hparams.highpass
        )
        ...
        if asds.shape[-1] != X_fft.shape[-1]:
            asds = F.interpolate(
                asds, size=X_fft.shape[-1], mode="linear", align_corners=False
            )
