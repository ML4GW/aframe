...
class Xylophone(torch.nn.Module):
    ...
    def __init__(
        ...
        octaves = [
            Octave(64, stride=1, dilation=64, layers=[2, 2, 3, 3]),
            Octave(32, stride=2, dilation=32, layers=[2, 2, 2, 3]),
            Octave(16, stride=3, dilation=16, layers=[2, 2, 2, 2]),
            Octave(8, stride=3, dilation=8, layers=[2, 4, 3, 2]),
            Octave(4, stride=3, dilation=4, layers=[2, 4, 3, 2]),
            Octave(1, stride=2, dilation=1, layers=[2, 4, 3, 2]),
        ]
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for i in range(num_octaves):
            xoct = self.octaves[f"octave{i}"](x)
